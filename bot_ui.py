...
def home_text(user: dict | None, *, is_new: bool) -> str:
    ...
    status = "🔔 вкл." if active else "⏸ пауза"
    ...
    if country == COUNTRY_RU:
        if not AVITO_ENABLED:
            ...
            hint = "➡️ Выберите город."
        elif models_n == 0:
            hint = "➡️ Выберите модель в «Товары»."
        elif not active:
            hint = "➡️ Включите уведомления."
    else:
        if models_n == 0:
            hint = "➡️ Выберите модель в «Товары»."
        elif not active:
    ...
    if is_new and not hint:
        hint = "➡️ Настройте фильтры и включите уведомления."
    if hint:
        lines += ["", hint]
...
def vip_text(user: dict | None) -> str:
    ...
    if user and user.get("role") == "vip":
        ...
        if ROLLYPAY_ENABLED:
            lines.append("Продлить срок — кнопка ниже.")
    else:
        lines += [
            "",
            "📱 без лимита моделей",
