...
async def present_home(msg: Message, state: FSMContext, *, register: bool) -> None:
    ...
    if register and is_new:
        ...
        if ref_code:
            process_referral_signup(chat_id, ref_code)
    if not register:
...
async def on_start(msg: Message, state: FSMContext) -> None:
    ...
    await present_home(msg, state, register=True)


@router.message(F.text)
async def on_any_text(msg: Message, state: FSMContext) -> None:
    await present_home(msg, state, register=get_user(msg.chat.id) is None)
